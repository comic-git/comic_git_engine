...
| Optional CMS output         | [`src/build/output/cms.py`](../src/build/output/cms.py), [`templates/cms/`](../templates/cms/) | Validates that the main TOML config and TOML pages can round-trip safely, then generates the static Decap admin entry point, Comic Settings form, and page collections. |
...
12. When site-wide CMS support is enabled, [`src/build/output/cms.py`](../src/build/output/cms.py) validates the main `comic_info.toml` and every main and Extra Comic page before generating marked `admin/index.html` and `admin/config.yml`. The generated config contains one creator-oriented main Comic Settings file collection plus the page collections. This happens after `site_root` copying and before the postprocess hook.
